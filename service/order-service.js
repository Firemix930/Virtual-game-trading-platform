    ...
    try {
        ...
        if (delSize == 1) {
            ...
            for (let i = 0; i < orderProds.length; i++) {
                await sequelize.query('UPDATE product SET stock = stock + :count, updatedAt = now(), version = version + 1 WHERE id = :id', { replacements: { count: orderProds[i].count, id: orderProds[i].prodId }, transaction: timeTransaction });
            }
...
module.exports = {
    ...
    settlementAct: async(ctx, cartIds, userIdIn) => {
        ...
        for (let i = 0; i < cartIdArr.length; i++) {
            ...
            await sequelize.query('UPDATE product SET stock = stock - :count, updatedAt = now(), version = version + 1 WHERE id = :id', { replacements: { count: qCart.count, id: qCart.prodId }, transaction: ctx.transaction });

            let qProduct = await product.findById(qCart.prodId, { transaction: ctx.transaction });
            if (qProduct.stock < 0) {
                let realStock = qProduct.stock + qCart.count;
                throw new APIError('settlement:lack_stock', qProduct.name + 'Only' + realStock + 'pieces');
            }
    ...
    buyNow: async(ctx, pid, pcount, userIdIn) => {
        ...
        await sequelize.query('UPDATE product SET stock = stock - :count, updatedAt = now(), version = version + 1 WHERE id = :id', { replacements: { count: pcountInt, id: pidInt }, transaction: ctx.transaction });

        let qProduct = await product.findById(pidInt, { transaction: ctx.transaction });
        if (qProduct.stock < 0) {
            let realStock = qProduct.stock + pcountInt;
            throw new APIError('settlement:lack_stock', qProduct.name + '' + realStock + 'piece');
        }
    ...
    orderDetail: async(orderId, userIdIn) => {
        ...
        if (orderStatus == 1) {
            qOrder.statusName = "Initial";
        } else if (orderStatus == 2) {
            qOrder.statusName = "Wait paying";
        } else if (orderStatus == 3) {
            qOrder.statusName = "Wait Sending";
        } else if (orderStatus == 4) {
            qOrder.statusName = "Wait recieving";
        } else if (orderStatus == 5) {
            qOrder.statusName = "Wait comment";
        } else if (orderStatus == 6) {
            qOrder.statusName = "Completed comment";
        }
    ...
    confirmOrder: async(ctx, orderId, userIdIn) => {
        ...
        if (qOrder) {
            ...
            if (qOrder.status != 1) {
                throw new APIError('settlement:invalid_order', 'Wrong order');
            }
        } else {
            throw new APIError('settlement:invalid_order', 'No order or invalid');
        }
        ctx.transaction = await sequelize.transaction();
        ...
        let uptRet = await sequelize.query('UPDATE `order` SET status = 2, payStartTime = :payStartTime, updatedAt = now(), version = version + 1 ' +
            'WHERE id = :id AND userId = :userId AND status = 1 AND version = :version', { replacements: { id: orderId, userId: userIdIn, version: qOrder.version, payStartTime: date }, transaction: ctx.transaction });
    ...
    confirmReceipt: async(ctx, orderId, userIdIn) => {
        ...
        let uptRet = await sequelize.query('UPDATE `order` SET status = 5, updatedAt = now(), version = version + 1 ' +
            'WHERE id = :id AND userId = :userId AND status = 4', { replacements: { id: orderId, userId: userIdIn } });
        if (uptRet[0].affectedRows != 1) {
            throw new APIError('order:invalid_order', 'No order or invalid');
        }
    ...
    payOrder: async(ctx, orderId, totalPrice, userIdIn) => {
        ...
        if (qOrder) {
            ...
            if (qOrder.totalPrice != totalPrice) {
                throw new APIError('settlement:error_totalPrice', 'Wrong price');
            }
        } else {
            throw new APIError('settlement:invalid_order', 'No order or invalid');
        }
        ...
        let uptRet = await sequelize.query('UPDATE `order` SET status = 3, updatedAt = now(), version = version + 1 ' +
            'WHERE id = :id AND userId = :userId AND status = 2 AND version = :version', { replacements: { id: orderId, userId: userIdIn, version: qOrder.version } });
        if (uptRet[0].affectedRows != 1) {
            throw new APIError('settlement:invalid_order', 'No order or invalid');
        }
    ...
    getOrderList: async(limit, offset, userIdIn, status) => {
        ...
        for (let i = 0; i < orders.length; i++) {
            ...
            let orderProds = await sequelize.query('SELECT p.id, p.`name`, p.price, p.image, op.count ' +
                ...
                'WHERE op.orderId = :orderId', { replacements: { orderId: orders[i].id }, type: sequelize.QueryTypes.SELECT });
            ...
            if (orderStatus == 1) {
                orderObj.statusName = "Initial";
            } else if (orderStatus == 2) {
                orderObj.statusName = "Wait paying";
            } else if (orderStatus == 3) {
                orderObj.statusName = "Wait sending";
            } else if (orderStatus == 4) {
                orderObj.statusName = "Wait receiving";
            } else if (orderStatus == 5) {
                orderObj.statusName = "Wait comment";
            } else if (orderStatus == 6) {
                orderObj.statusName = "Complete comment";
            }
