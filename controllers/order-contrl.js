...
module.exports = {
    ...
    'POST /userapi/settlementAct': async(ctx, next) => {
        ...
        if (!cartIds || cartIds.trim() == '') {
            throw new APIError('settlement:empty_cartIds', 'cartIds cannot be empty');
        }
    ...
    'POST /userapi/buyNow': async(ctx, next) => {
        ...
        let userId = ctx.session.user.userId;
        if (!pid || pid.trim() == '') {
            throw new APIError('settlement:empty_pid', 'pid cannot be empty');
        }
        if (!pcount || pcount.trim() == '' || parseInt(pcount) <= 0) {
            throw new APIError('settlement:invalid_pcount', 'Invalidpcount');
        }
    ...
    'POST /userapi/confirmOrder': async(ctx, next) => {
        ...
        let userId = ctx.session.user.userId;
        if (!orderId || orderId.trim() == '') {
            throw new APIError('settlement:empty_orderId', 'orderId cannot be empty');
        }
    ...
    'POST /userapi/confirmReceipt': async(ctx, next) => {
        ...
        let userId = ctx.session.user.userId;
        if (!orderId || orderId.trim() == '') {
            throw new APIError('order:empty_orderId', 'orderId cannot be empty');
        }
    ...
    'POST /userapi/payOrder': async(ctx, next) => {
        ...
        if (!totalPrice || totalPrice.trim() == '') {
            throw new APIError('settlement:empty_totalPrice', 'totalPrice cannot be empty');
        }
