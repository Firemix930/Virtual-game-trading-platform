 ...
 * @LastEditTime: 2020-11-12 11:56:36
 * @Description: file that implements the product details function
 * @FilePath: \VSLibrary\Team Work E\static\js\app\prod-detail.js
 */
...
requirejs(["jquery", "publicTip", "swiper-4.1.6.min", "jquery.Spinner"], function($, publicTip, Swiper) {
    $(function() {
        ...
        $("#addCart").click(function() {
            ...
            if (stock <= 0) {
                publicTip.showAlert("Inventory is empty");
                return;
            }
            publicTip.showLoadingToast(true, "Joining");
            $.ajax({
                type: 'post',
                dataType: 'json',
                ...
                publicTip.showToast("Has joined");
        ...
        $("#collectionHref").click(function() {
            ...
            var msg = 'Cancel the collection';
            if (id.trim() == '') msg = 'Collection';
            publicTip.showLoadingToast(true, msg + "ing");
            $.ajax({
                type: 'post',
                dataType: 'json',
                ...
                if (r.id) {
                    ...
                    //$("#collectionHref").find("p").html('Cancel the collection');
                    ...
                    //$("#collectionHref").find("p").html('collection');
                    $("#collectionHref").find("p").css("color", "gray");
                }
                publicTip.showLoadingToast(false);
                publicTip.showToast("already" + msg);
            }).fail(function(jqXHR, textStatus) {
                publicTip.showLoadingToast(false);
                publicTip.showTip(jqXHR.responseJSON);
            });
